#include "MiliEngine.h"

MiliEngine::MiliEngine()
{
	m_nScreenWidth = 80;
	m_nScreenHeight = 30;
...
void MiliEngine::EnableSound()
{
	m_bEnableSound = true;
}

int MiliEngine::ConstructConsole(int width, int height, int fontw, int fonth)
{
	...
	/////////////// renderer goes here ////////////////
	renderer = new Renderer(m_nScreenWidth, m_nScreenHeight);
...
void MiliEngine::Start()
{
	// Start the thread
	m_bAtomActive = true;
	std::thread t = std::thread(&MiliEngine::GameThread, this);

	// Wait for thread to be exited
	t.join();
}

int MiliEngine::ScreenWidth()
{
	return m_nScreenWidth;
}

int MiliEngine::ScreenHeight()
{
...
///////////// private methods start her ///////////////

void MiliEngine::GameThread()
{
	...
	while (m_bAtomActive)
	{
		...
		while (m_bAtomActive)
		{
			...
			wchar_t s[256];
			swprintf_s(s, 256, L"MiliSick - Console Game Engine - %s - FPS: %3.2f", m_sAppName.c_str(), 1.0f / fElapsedTime);
			SetConsoleTitle(s);
			WriteConsoleOutput(m_hConsole, renderer->GetBufScreen(), { (short)m_nScreenWidth, (short)m_nScreenHeight }, { 0, 0 }, &m_rectWindow);
		}
...
//////////////// game thread ends here ////////////////////


///////////////////// protected methods ////////////////

int MiliEngine::Error(const wchar_t *msg)
{
	wchar_t buf[256];
...
BOOL MiliEngine::CloseHandler(DWORD evt)
{
...
////////////// events /////////////
	...
	MiliEngine::sKeyState MiliEngine::GetKey(int nKeyID){ return m_keys[nKeyID]; }
	int MiliEngine::GetMouseX() { return m_mousePosX; }
	int MiliEngine::GetMouseY() { return m_mousePosY; }
	MiliEngine::sKeyState MiliEngine::GetMouse(int nMouseButtonID) { return m_mouse[nMouseButtonID]; }
	bool MiliEngine::IsFocused() { return m_bConsoleInFocus; }
	...
	unsigned int MiliEngine::LoadAudioSample(std::wstring sWavFile)
	{
		if (!m_bEnableSound)
			return -1;

		MiliAudio a(sWavFile);
		if (a.bSampleValid)
		{
	...
	void MiliEngine::PlaySample(int id, bool bLoop = false)
	{
	...
	void MiliEngine::StopSample(int id)
	{

	}

	// The audio system uses by default a specific wave format
	bool MiliEngine::CreateAudio(unsigned int nSampleRate, unsigned int nChannels,
		unsigned int nBlocks , unsigned int nBlockSamples )
	{
		...
		m_AudioThread = std::thread(&MiliEngine::AudioThread, this);
	...
	void MiliEngine::waveOutProc(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwParam1, DWORD dwParam2)
	{
	...
	void CALLBACK MiliEngine::waveOutProcWrap(HWAVEOUT hWaveOut, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2)
	{
		((MiliEngine*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
	...
	float MiliEngine::onUserSoundSample(int nChannel, float fGlobalTime, float fTimeStep)
	{
		return 0.0f;
	}

	// Overriden by user if they want to manipulate the sound before it is played
	float MiliEngine::onUserSoundFilter(int nChannel, float fGlobalTime, float fSample)
	{
		return fSample;
	}
	...
	float MiliEngine::GetMixerOutput(int nChannel, float fGlobalTime, float fTimeStep)
	{
	...
	std::atomic<bool> MiliEngine::m_bAtomActive(false);
	std::condition_variable MiliEngine::m_cvGameFinished;
	std::mutex MiliEngine::m_muxGame;